...
async function playMoveOnServer(fen, san, rating) {
    console.log(`🌐 [Client] Запрос /play-move для "${san}" к ${API_BASE || 'origin'}...`);
    ...
    try {
        ...
        if (!response.ok) {
            console.warn(`⚠️ [Client] /play-move HTTP ошибка ${response.status} (${elapsed}ms)`);
            return { check: { inBook: false, rank: 99 }, reply: null, gameOver: false };
        }
        const data = await response.json();
        console.log(`✅ [Client] /play-move получен ответ за ${elapsed}ms:`, data);
        return data;
    } catch (e) {
        const elapsed = Math.round(performance.now() - t0);
        console.error(`❌ [Client] /play-move упал по ошибке/таймауту (${elapsed}ms):`, e.message);
        return { check: { inBook: false, rank: 99 }, reply: null, gameOver: false };
    }
}
